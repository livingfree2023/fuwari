...
upload: false
Last Modified: 2026-07-29 13:07:12
---

假如局域网LAN中有一台NAS或者限制的主机可以跑docker，那么就可以获得一个24小时在线的旁路由。
...
如果是linux，用`docker compose up -d` 启动，其他OS问AI。 

然后浏览器访问`https://metacubex.github.io/metacubexd` endpoint的ip填入当前机器的I （比如`http://192.168.0.100`)，端口默认`9090` 应该就可以进行控制了，非常方便。比部署虚拟机旁路由占用资源低多了。
